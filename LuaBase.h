...
// Gkit will use lua_State defined in Interface.h
namespace GKit
{
	namespace GLua
	{
...
namespace GarrysMod
{
	namespace Lua
	{
		typedef int(*CFunc) (GKit::GLua::lua_State *L);
		...
		public:
			...
			virtual int			Top(void) = 0; //calls lua_gettop nothing else
			virtual void		Push(int iStackPos) = 0; //calls lua_pushvalue nothing else
			virtual void		Pop(int iAmt = 1) = 0; //calls lua_settop
			...
			//
			virtual void		PushSpecial(int iType) = 0; //pushes 0FFFFD8EEh or 0FFFFD8EFh or 0FFFFD8F0h then does weird thing
